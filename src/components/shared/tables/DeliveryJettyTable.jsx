// src/components/tables/DeliveryJettyTable.jsx

import React, { useState } from "react";
import { FaEdit, FaSave, FaTimes } from "react-icons/fa";

const DeliveryJettyTable = ({ 
  items = [], 
  onEditItem,
  ...
  const handleDeliveryQuantityChange = (index, value) => {
    ...
    newItems[index].deliveryjettyDeliveredQuantity = parsedValue;
      ...
      <table className="w-full border-collapse border-2 border-slate-200 rounded-lg overflow-hidden">
        ...
        <tbody>
          {editedItems.map((item, index) => {
            ...
            const deliveredQty = parseInt(item.deliveryjettyDeliveredQuantity) || 0;
                ...
                <td className="border border-slate-200 px-4 py-3 text-center">
                  {editingIndex === index ? (
                    <input
                      ...
                      className="w-20 px-2 py-1 border-2 border-teal-300 rounded-md text-center focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-teal-500"
                    />
                  ) : (
                    <span className="font-semibold text-teal-600">{deliveredQty}</span>
                  )}
                </td>
                <td className="border border-slate-200 px-4 py-3 text-center">
                    ...
                    <div className="flex items-center justify-center gap-2">
                      {editingIndex === index ? (
                        ...
                        <button
                          ...
                          className="p-2 text-teal-600 hover:bg-teal-50 rounded-lg transition-colors duration-150"
...
export default DeliveryJettyTable;