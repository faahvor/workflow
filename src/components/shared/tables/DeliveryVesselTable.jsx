// src/components/tables/DeliveryVesselTable.jsx

import React, { useState } from "react";
import { FaEdit, FaSave, FaTimes } from "react-icons/fa";

const DeliveryVesselTable = ({ 
  items = [], 
  onEditItem,
  ...
  const handleDeliveryQuantityChange = (index, value) => {
    ...
    newItems[index].deliveryvesselDeliveredQuantity = parsedValue;
      ...
      <table className="w-full border-collapse border-2 border-slate-200 rounded-lg overflow-hidden">
        ...
        <tbody>
          {editedItems.map((item, index) => {
            ...
            const deliveredQty = parseInt(item.deliveryvesselDeliveredQuantity) || 0;
                ...
                <td className="border border-slate-200 px-4 py-3 text-center">
                  {editingIndex === index ? (
                    <input
                      ...
                      className="w-20 px-2 py-1 border-2 border-indigo-300 rounded-md text-center focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                    />
                  ) : (
                    <span className="font-semibold text-indigo-600">{deliveredQty}</span>
                  )}
                </td>
                <td className="border border-slate-200 px-4 py-3 text-center">
...
export default DeliveryVesselTable;