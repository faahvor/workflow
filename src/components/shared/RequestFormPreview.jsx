...
const RequestFormPreview = forwardRef(
 ...
 const fetchImageAsDataUrl = async (url) => {
    ...
    const requiredDate = req.requiredDate || new Date(createdAt.getTime() + 24 * 60 * 60 * 1000).toLocaleDateString();
        ...
        <div className="bg-white/90 border-2 border-slate-200 rounded-2xl overflow-hidden mb-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4">
            <div className="px-4 py-3 border-b border-r border-slate-200">
              ...
              <p className="text-sm text-slate-900 font-semibold font-mono">{reqIdStr}</p>
            </div>
             <div className="px-4 py-3 border-b border-r border-slate-200">
              <p className="text-xs text-slate-500 font-medium mb-0.5">Company</p>
              <p className="text-sm text-slate-900 font-semibold">{(req.company && req.company.name) || req.companyName || "HWFP Marine Services"}</p>
            </div>

            <div className="px-4 py-3 border-b border-r border-slate-200">
              <p className="text-xs text-slate-500 font-medium mb-0.5">Requester</p>
              <p className="text-sm text-slate-900 font-semibold">
                {typeof req.requester === "object" ? req.requester.displayName || req.requester.name : req.requester || "N/A"}
              </p>
            </div>

            <div className="px-4 py-3 border-b border-r border-slate-200">
              <p className="text-xs text-slate-500 font-medium mb-0.5">Department</p>
              <p className="text-sm text-slate-900 font-semibold">{req.department || "N/A"}</p>
            </div>

            <div className="px-4 py-3 border-b border-r border-slate-200">
              <p className="text-xs text-slate-500 font-medium mb-0.5">Destination</p>
              <p className="text-sm text-slate-900 font-semibold">{req.destination || req.destinationPort || "N/A"}</p>
            </div>

            <div className="px-4 py-3 border-b border-r border-slate-200">
              <p className="text-xs text-slate-500 font-medium mb-0.5">Vessel</p>
              <p className="text-sm text-slate-900 font-semibold">{req.vessel || req.vesselName || req.vesselId || "N/A"}</p>
            </div>
...
<div className="px-4 py-3 border-b border-r border-slate-200">
              ...
              <p className="text-sm font-semibold">
                <span className="inline-block px-2 py-0.5 rounded text-xs bg-emerald-100 text-emerald-700">
                  {req.requestType === "purchaseOrder" || req.type === "purchase-order" ? "Purchase Order" : "Petty Cash"}
                </span>
              </p>
            </div>
              <div className="px-4 py-3 border-r border-slate-200">
              <p className="text-xs text-slate-500 font-medium mb-0.5">Asset ID</p>
              <p className="text-sm text-slate-900 font-semibold">{req.assetId || "N/A"}</p>
            </div>

            <div className="px-4 py-3 border-b border-r border-slate-200">
              <p className="text-xs text-slate-500 font-medium mb-0.5">Priority</p>
              <p className="text-sm font-semibold">
                <span className={`inline-block px-2 py-0.5 rounded text-xs ${String(req.priority || "").toLowerCase() === "urgent" ? "bg-red-100 text-red-700" : "bg-slate-100 text-slate-700"}`}>
                  {String(req.priority || "").toLowerCase() === "urgent" ? "URGENT" : req.priority || "Normal"}
                </span>
              </p>
            </div>
            <div className="px-4 py-3 border-r border-b  border-slate-200">
              <p className="text-xs text-slate-500 font-medium mb-0.5">Logistics Type</p>
              <p className="text-sm text-slate-900 font-semibold capitalize">{req.logisticsType || "N/A"}</p>
            </div>
            <div className="px-4 py-3 border-r border-b  border-slate-200">
              <p className="text-xs text-slate-500 font-medium mb-0.5">Job Number</p>
              <p className="text-sm text-slate-900 font-semibold capitalize">{req.jobNumber || "N/A"}</p>
            </div>
          ...
          <div className="bg-white border-2 border-slate-200 rounded-2xl p-3">
            <div className="overflow-x-auto">
              <table className="w-full text-sm border-collapse">
                ...
                <tbody>
                  {usedItems.map((it, i) => (
                    <tr key={it.itemId || it.id || i} className="border-b last:border-b-0 border-slate-200">
                      <td className="px-3 py-2 align-top">{i + 1}</td>
                      <td className="px-3 py-2 align-top text-slate-900">{it.name}</td>
                      <td className="px-3 py-2 align-top">{it.quantity}</td>
                      <td className="px-3 py-2 align-top">{it.unit || "N/A"}</td>
                      <td className="px-3 py-2 align-top text-right text-slate-700">{formatCurrency(it.unitPrice, req.currency)}</td>
                      <td className="px-3 py-2 align-top text-right font-semibold text-slate-900">{formatCurrency(it.total || (Number(it.unitPrice || 0) * Number(it.quantity || 0)), req.currency)}</td>
                    </tr>
                  ))}
                </tbody>
